      ...
      <div className="max-w-6xl px-4 mx-auto sm:px-6">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <nav className="hidden md:flex md:grow">
            ...
            <ul className="flex flex-wrap items-center justify-center grow">
              {links?.map((link, index) => (
                <li key={link.label}>
                  <Link className="mx-4 text-sm font-medium transition duration-150 ease-in-out text-slate-300 hover:text-white lg:mx-5" href={link.link}>{link.label}</Link>
                </li>
              ))}
            </ul>
          ...
          <ul className="flex items-center justify-end flex-1">
            <li>
              <Link className="text-sm font-medium transition duration-150 ease-in-out text-slate-300 hover:text-white whitespace-nowrap" href="/signin">Sign in</Link>
            </li>
            <li className="ml-6">
